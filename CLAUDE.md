...
| 4 | **Fresh-final / draft-frame audit** — the completed Telegram reply is delivered by `_try_fresh_final` (a NEW message + delete of the stale streaming preview) and by native draft frames. Suspected badge bypass. | `gateway/stream_consumer.py::_try_fresh_final` (~L1416), `_send_draft_frame` (~L1201) | **Verified NOT bypassed**: both receive text already badged by `_send_or_edit` (~L1528, prepend at the top). Regression-pinned in `tests/gateway/test_stream_consumer_badge_send_paths.py`. |
...
venv/bin/python -m pytest \
  ...
  tests/gateway/test_stream_consumer_badge_send_paths.py -q
```
