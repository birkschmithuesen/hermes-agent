...
def build_context_files_prompt(
    ...
    AGENTS.md from HERMES_HOME (operational policy) is likewise independent and always included when
    present — prepended BEFORE the cwd-local project context so project AGENTS.md can override/augment
    the baseline policy.
    """
    cwd_path = Path(cwd if cwd is not None else os.getcwd()).resolve()
    # Operational policy from HERMES_HOME — cwd-independent baseline. This is an explicit, config-rooted
    # load (not cwd-derived), so the install-tree fallback guard below deliberately does NOT apply to it:
    ...
    # chain).
    #
    # CAVEAT — this ASSUMES no .hermes.md/HERMES.md sits in HERMES_HOME.  That rung (priority 1,
    # walk-to-git-root) outranks AGENTS.md, so if one existed here it would be dropped by this skip,
    # whereas pre-change it would have won the chain.  No such file exists in this profile (verified), so
    # the skip keeps the old outcome; revisit this if a .hermes.md is ever added at the profile root.
    # Note this is unrelated to the vault's per-directory AGENTS.md, which load via
    # agent/subdirectory_hints.py, not this chain.
    ...
    # HERMES_HOME policy comes first so cwd-local project context can override/augment it.
    sections = ([home_agents] if home_agents else []) + sections
    if not skip_soul:
