...
booked as a protocol violation — `77` (`EX_NOPERM`) when the provider
refused the profile's login (401/403, revoked or invalid key, or a 5xx whose
body is an auth error, such as the `anthropic_plan` proxy after its Claude
login expired) or credential resolution says a re-login is required — the
dispatcher records the run as `auth_failed`, returns the card to `ready`
without counting a failure and holds it on the `auth_failed_cooldown` (see
[Respawn guard](#respawn-guard)) until someone renews the login (for
`anthropic_plan`: `claude /login` on the host); every waiting card then resumes
...
missing model parks the card exactly like an implementer. `hermes kanban show`
surfaces it as *Terminal provider error (model, TLS or upstream block) —
blocked after one attempt*; fix the assignee profile's provider/model
(`hermes -p <profile> setup`), then `hermes kanban unblock <id>`. The worker also writes its exit code as the
...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR (`active_pr`). Two cooldowns hold a card without ever counting against it: `rate_limit_cooldown` after a quota-wall requeue and `infrastructure_cooldown` after the host refused to place the worker (no restart-safe systemd scope — see [Workers and systemd cgroups](#workers-and-systemd-cgroups)); both share the `HERMES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS` window (default 300 s). A third, `auth_failed_cooldown`, holds a card whose last run exited `77` (the provider refused the profile's login) for `HERMES_KANBAN_AUTH_FAILED_COOLDOWN_SECONDS` (default 1800 s), also never counted; it takes precedence over `blocker_auth`, so the card retries by itself after each window until the login is renewed. This prevents repeat worker storms on the same bug or task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).

To hear about a logged-out profile, set an alert target in the **dispatcher's** profile config (the gateway or `hermes kanban dispatch` process that runs the tick, not the failing worker profile):

```yaml
kanban:
  auth_alert:
    target: "telegram:<chat_id>:<thread_id>"   # default "" = no alert
```

The target uses the `send_message` format. The dispatcher sends one message per outage episode per assignee profile, naming the profile, the number of waiting cards and the fix. A later run of that profile ending any other way closes the episode, so the next outage alerts again.

To see why a ready card is not spawning, run `hermes kanban dispatch --dry-run` — the output lists `Guarded (<reason>): <task id>` per held card (and `respawn_guarded`, `rate_limited`, `auth_failed`, `skipped_locked`, `memory_pressure` with `--json`). The gateway's and the standalone daemon's "dispatcher stuck" warning also names what the last tick held back, e.g. `Last tick held back: active_pr=1`.
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `infrastructure_cooldown` (the host refused the last spawn — no restart-safe systemd scope — and the cooldown has not elapsed; never counted against the card), `rate_limit_cooldown` (the last run hit a quota wall; same cooldown, never counted), `auth_failed_cooldown` (the last run exited `77` because the provider refused the profile's login; 30-minute cooldown, never counted), `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a GitHub PR URL appears in a recent comment — a prior worker already opened a PR). The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
...
| `gave_up` | `{failures, effective_limit, limit_source, error, terminal_provider?}` | Circuit breaker fired after N consecutive non-successful attempts. Task auto-blocks with the last error. The effective limit resolves as task `max_retries`, then dispatcher `failure_limit` / `kanban.failure_limit`, then the built-in default. `terminal_provider: true` means the worker exited `78` on a provider error a retry cannot fix (model gone, TLS chain broken, upstream block) and the breaker fired on that first attempt, sticky, regardless of the limit. A logged-out credential is not terminal here — it exits `77` (`KANBAN_AUTH_FAILED_EXIT_CODE`) instead, and the card stays `ready` on the auth cooldown rather than blocking. |
